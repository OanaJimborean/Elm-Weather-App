...
toHourlyDataPoints apiWeatherData =
    let
        hData = apiWeatherData.hourly
        utcOff = apiWeatherData.utcOffset
    in
        List.map3 (\x y z -> HourlyDataPoint (Time.millisToPosix ((x + utcOff) * 1000)) y z) hData.times hData.temperatures hData.precipitation
...
decodeWeatherData = De.map2 ApiWeatherData
